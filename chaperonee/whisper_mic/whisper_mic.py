...
class WhisperMic:
    def __init__(self,model="base",device=("cuda" if torch.cuda.is_available() else "cpu"),english=False,verbose=False,energy=300,pause=0.8,dynamic_energy=False,save_file=True):
...
#########
    ...
    def transcribe(self,data=None,realtime=False):
        ...
        if self.english:
            result = self.audio_model.transcribe(audio_data,language='French')
        else:
